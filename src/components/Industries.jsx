          ...
          <div className="flex-1">
            ...
            <div className="space-y-3">
              {industries.map((industry) => {
                ...
                return (
                  <div key={industry.id} className="border rounded-xl bg-white shadow-sm transition-all duration-300">
                    <button
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <span className="text-blue-600">{iconMap[industry.icon] || null}</span>
                        <span className="font-semibold text-base text-gray-800">{industry.label}</span>
                      </div>
                    ...
                    {isOpen && (
                      <div className="px-6 pb-4 pt-1 animate-fade-in">
                        <div className="text-sm text-gray-700 mb-2 font-medium">{industry.content.title}</div>
                        <div className="text-xs text-gray-500 mb-3">{industry.content.description?.slice(0, 120)}...</div>
                        <button
        ...
        >
          ...
          >
            ...
            <div className="bg-gradient-to-r from-blue-700 to-blue-900 rounded-t-3xl p-7 text-white relative flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-2xl font-bold mb-1 leading-tight">{modalIndustry.content.title}</h3>
                <p className="text-gray-200 text-sm">{modalIndustry.label} Solutions</p>
              </div>
              <button 
                onClick={closeModal}
            ...
            <div className="p-7 bg-white rounded-b-3xl">
              <div className="mb-5">
                <p className="text-gray-700 leading-relaxed text-base">{modalIndustry.content.description}</p>
              </div>
              {/* Features */}
              <div className="mb-6">
                <h4 className="text-lg font-semibold text-gray-800 mb-3">Key Features & Benefits</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {modalIndustry.content.features.map((feature, index) => (
                    <div key={index} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-all duration-300 border border-gray-100">
